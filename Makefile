...
ADD_NO_ITEM_TARGET := $(BINDIR)/add-no-item
...
COMPLETION_ADD_NO_ITEM_TARGET := $(COMPLETIONSDIR)/add-no-item
...
install:
	...
	ln -sf "$(NO_ITEM_SCRIPT)" "$(ADD_NO_ITEM_TARGET)"
	install -d "$(COMPLETIONSDIR)"
	ln -sf "$(COMPLETION_SCRIPT)" "$(COMPLETION_TARGET)"
	ln -sf "$(COMPLETION_SCRIPT)" "$(COMPLETION_NO_ITEM_TARGET)"
	ln -sf "$(COMPLETION_SCRIPT)" "$(COMPLETION_ADD_NO_ITEM_TARGET)"

uninstall:
	rm -f "$(ADD_TARGET)" "$(NO_ITEM_TARGET)" "$(ADD_NO_ITEM_TARGET)" "$(COMPLETION_TARGET)" "$(COMPLETION_NO_ITEM_TARGET)" "$(COMPLETION_ADD_NO_ITEM_TARGET)"
